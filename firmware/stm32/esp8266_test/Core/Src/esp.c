...
//================== USART1 (ESP8266 AT) ====================
//  ★ 변경 포인트: ESP8266은 USART1 사용
extern UART_HandleTypeDef huart1;
static uint8_t data;                // ESP 수신 1바이트 버퍼
cb_data_t cb_data;                  // ESP 수신 링버퍼(라인 파싱)
...
static int esp_at_command(uint8_t *cmd, uint8_t *resp, uint16_t *length, int16_t time_out)
{
    *length = 0;
    ...
    if (HAL_UART_Transmit(&huart1, cmd, strlen((char *)cmd), 100) != HAL_OK)   // ★ huart1
        return -1;
